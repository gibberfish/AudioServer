...
private Jukebox jukebox;
  ...
  public void shouldThrowExceptionIfAttemptToGetArtworkForAlbum () {
	  // Given
	  when (mockMediaPlayerCache.getMediaItemWithId(23)).thenReturn(mockAlbum);
  ...
  public void shouldReturnArtworkForTrack () {
	  // Given
	  when (mockMediaPlayerCache.getMediaItemWithId(23)).thenReturn(mockTrack);
	  when (mockTrack.getArtworkFile()).thenReturn(ART_FILE);
  ...
  public void shouldBroadcastAMessageWhenASongStartsToPlay () {
	  ...
	  // Then
	  verify (mockStatusBroadcaster).broadcast(eq(PlayerStatus.PLAYING.toString()), eq(Jukebox.NO_PLAYLIST), eq(false), eq(false), eq(""));
  }
